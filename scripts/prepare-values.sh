...
yq eval -i ".template.spec.containers[0].command[0] = \"cmd.exe\"" "$OUTPUT_FILE"
yq eval -i ".template.spec.containers[0].command[1] = \"/c\"" "$OUTPUT_FILE"
yq eval -i ".template.spec.containers[0].command[2] = \"\\\\\\\\home\\\\\\\\runner\\\\\\\\run.cmd\"" "$OUTPUT_FILE"
